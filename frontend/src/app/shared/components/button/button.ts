...
type ButtonVariant = 'primary' | 'myColor' | 'destructive' | 'secondary' | 'ghost' | 'link' | 'icon';
type ButtonSize = 'sm' | 'md' | 'lg';

const variantClasses: Record<ButtonVariant, string> = {
  primary: 'bg-slate-900 text-white hover:bg-slate-800 focus:ring-slate-900',
  myColor: 'bg-slate-100',
...
export class Button {
  ...
  protected readonly hostClasses = computed(()=> {
    const base = 'inline-flex cursor-pointer items-center justify-center rounded-md font-medium transition-colors focus:outline-none disabled:opacity-50 disabled:pointer-events-none ring-offset-white dark:ring-offset-slate-900';
